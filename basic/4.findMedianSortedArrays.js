...
var findMedianSortedArrays2 = function (nums1, nums2) {
  /**
   * 1. 정렬된 배열 2개의 전체 길이를 찾는다.
   * 2. 배열을 전체의 절반 길이까지 순서만 찾는다
   * 3. 홀수면 마지막 1개 짝수면 마지막 2개의 값을 가져와서 중앙값을 찾는다
   *
   * 1. Find the total length of the two sorted arrays.
   * 2. Identify the elements up to half the total length of the arrays.
   * 3. Take the last num if the total length is odd, or the last two nums if even, to find the median.
   */
  ...
  return isOdd ? curData : (curData + beforeData) / 2;
};
